...
export default function ExamRoom({
  ...
  const [activeExam, setActiveExam] = useState(FALLBACK_EXAM);
  ...
  const [finishState, setFinishState] = useState({ finishing: false, finished: false });
  ...
  const handleFinishExam = useCallback(async (status = "submitted") => {
    ...
    const finishEventType = status === "terminated" ? "SESSION_TERMINATED" : "SESSION_COMPLETED";
    const finishMessage = status === "terminated"
    ...
    const riskScore = eventRisk?.risk_score ?? proctoring.riskData?.risk_score ?? 0;
    const riskLevel = eventRisk?.risk_level ?? proctoring.riskData?.risk_level ?? "LOW";
    const eventCounts = eventRisk?.event_counts ?? proctoring.riskData?.event_counts ?? {};
    ...
    try {
      await examService.finishSession(activeExam?._id || FALLBACK_EXAM._id, sessionId, {
        answers,
        riskScore,
        riskLevel,
        eventCounts,
  ...
  const handleStartProctoring = useCallback(async () => {
    ...
    try {
      ...
      setRemainingSeconds(session.remainingSeconds || (exam.duration || FALLBACK_EXAM.duration) * 60);
      setAnswers(session.answers || {});

      let fetchedQuestions = startResponse?.questions || [];
      if (!fetchedQuestions.length && exam?._id && exam._id !== FALLBACK_EXAM._id) {
        try {
          const questionsRes = await fetch(`/api/exams/${exam._id}/questions`).then((r) => r.json());
          fetchedQuestions = questionsRes.questions || [];
        } catch (err) {
          console.warn("Sorular çekilemedi:", err.message);
        }
      }
      setQuestions(fetchedQuestions.length ? fetchedQuestions : FALLBACK_QUESTIONS);
  ...
  useEffect(() => {
    if (!autoStart || autoStartedRef.current || proctoringStarted || loading) return;
  ...
  useEffect(() => {
    if (!proctoringStarted) return undefined;
    ...
    const handleContextMenu = (event) => reportBrowserEvent("RIGHT_CLICK_ATTEMPT", event);
    const handleCopyPaste = (event) => reportBrowserEvent("COPY_PASTE_ATTEMPT", event);
  ...
  const examBlocked = proctoringStarted && (!proctoring.cameraActive || !faceDetected);
  const blockMessage = !proctoring.cameraActive
  ...
  const handleOptionSelect = useCallback((optIndex) => {
    ...
    setAnswers((prev) => ({ ...prev, [currentQuestionId]: optIndex }));
  ...
  if (finishState.finished) {
    ...
    const handleReturn = () => {
      ...
      onNavigate(role === "admin" || role === "instructor" ? "instructor-dashboard" : "student-home");
    };
    ...
    return (
      <div style={{
        ...
        <div style={{ backgroundColor: "rgba(0, 200, 83, 0.1)", border: "1px solid #00c853", borderRadius: "10px", padding: "3rem", maxWidth: "600px", boxShadow: "0 10px 30px rgba(0,0,0,0.5)" }}>
          <h1 style={{ color: "#00c853", marginBottom: "1rem", fontSize: "2.5rem" }}>Sınav Bitti</h1>
          <p style={{ fontSize: "1.1rem", marginBottom: "2.5rem", color: "#ccc", lineHeight: "1.6" }}>
            Cevaplarınız ve değerlendirme raporlarınız başarıyla sistemimize kaydedildi. Katılımınız için teşekkür ederiz.
          </p>
          <button 
            onClick={handleReturn}
            style={{ padding: "1rem 2rem", fontSize: "1.1rem", backgroundColor: "#3f8cf4", color: "white", border: "none", borderRadius: "5px", cursor: "pointer", fontWeight: "bold", width: "100%" }}>
            Ana Sayfaya Dön
          </button>
        </div>
      </div>
        ...
        <div className="exam-layout">
          <header className="exam-header exam-header--with-nav">
            <div className="exam-header-left">
              ...
              <div>
                ...
                <div className="exam-user-text">{currentUser?.name || "Öğrenci"}</div>
              </div>
            </div>
            <nav className="exam-navbar">
              <button className="exam-navbar-link exam-navbar-link--active" type="button">Sınav</button>
              <button
              ...
              {onLogout && (
                <button className="exam-navbar-link" type="button" onClick={onLogout}>Çıkış</button>
              )}
            </nav>
          </header>
          <div className="exam-start-screen">
            ...
            <p className="exam-start-copy">
              Kamera, mikrofon, tam ekran ve yüz doğrulaması tamamlandıktan sonra sınav başlar.
            </p>

            {!precheckPassed && (
              <p className="exam-start-note">
                Gerçek sınav için önce sınav öncesi kontrol ekranından geçmeniz gerekir.
              </p>
            )}
        ...
        <div className="exam-layout">
          <header className="exam-header exam-header--with-nav">
            <div className="exam-header-left">
              <div className="exam-logo">AI</div>
              <div>
                <div className="exam-title-text">Sınav: {activeExam?.title || FALLBACK_EXAM.title}</div>
                <div className="exam-user-text">{currentUser?.name || "Öğrenci"}</div>
              </div>
            </div>

            <nav className="exam-navbar">
              <button className="exam-navbar-link exam-navbar-link--active" type="button">Sınav Ekranı</button>
              <button className="exam-navbar-link" type="button" disabled title="Sınav devam ederken panel kapalıdır">Öğrenci Paneli</button>
            </nav>
          ...
          <div className="exam-status-bar">
            ...
            <StatusIndicator type="face" status={faceDetected ? "active" : "inactive"} />
            <StatusIndicator
              type="gaze"
              status={proctoring.gazeResult?.gaze === "screen" ? "active" : "inactive"}
              detail={
                proctoring.gazeResult
                  ? `Bakış: ${proctoring.gazeResult.gaze === "screen" ? "Ekranda" : "Ekran dışı"}`
                  : undefined
              }
            />
            <StatusIndicator type="fullscreen" status={proctoring.isFullscreen ? "active" : "inactive"} />
            <span className="violation-pill">İhlal: {totalViolations} / {maxViolations}</span>
          </div>
          ...
          <main className={`exam-main ${examBlocked ? "exam-main--blocked" : ""}`}>
            {questions.length > 0 ? (
              ...
              <div className="question-card question-card--empty">Yükleniyor...</div>
            )}
          </main>
              ...
              <div className="modal-box">
                <div className="modal-header">
                  ...
                  <h3 className="modal-title">
                    {warning.terminating ? "Sınav Sonlandırılıyor" : "Kural İhlali"}
                  </h3>
                </div>
                <div className="modal-body">
                  {warning.messages.map((message) => (
                    <p className="modal-line" key={message}>{message}</p>
                  ))}
                  <p className="modal-violation">
                    İhlal sayısı: {warning.count} / {maxViolations}
                  </p>
                  {warning.terminating && (
                    <p className="modal-line">Maksimum ihlal sayısına ulaşıldığı için oturum kapatılıyor.</p>
                  )}
                </div>
                {!warning.terminating && (
                  <button
