      ...
      <main className="report-main">
        <div className="report-card">
          ...
          <div className="report-actions">
            <button className="report-btn report-btn--json" onClick={() => {
              const blob = new Blob([JSON.stringify({ report, eventCounts }, null, 2)], { type: "application/json" });
              const url = URL.createObjectURL(blob);
              window.open(url, "_blank");
            }}>JSON Dışa Aktar</button>
            <button className="report-btn report-btn--view" onClick={() => onNavigate("admin-dashboard")}>Oturumları Göster</button>
          </div>
        </div>
      </main>
    </div>
